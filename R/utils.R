...
dmv_gaussian <- function(X, m, s) {
    ...
    sig_inv <- solve(s)
    density_denominator <-
        (2 * pi) ^ (ncol(X) * 0.5) * (det(s)) ^ 0.5
    f <-
        apply(X, 1, function(x)
            exp(-0.5 * t(x - m) %*% sig_inv %*% (x - m)))
...
plot_multinomial <- function(dataQuali, ak) {
    ...
    for (k in seq_len(K)) {
        for (j in seq_len(ncol(ak[[k]]))) {
            #Sys.sleep(2)
            barplot(
                ak[[k]][complete.cases(ak[[k]][, j]), j],
                beside = T,
                col = color[nrow(ak[[k]]) + j:nrow(ak[[k]]) + j + j],
...
plot_gaussian <- function(D, cls, m, s, axis) {
    #browser()
    # if (ncol(D)>2){
    #
    # }
  ...
  if (ncol(D) == 1) {
        ...
        m <-
            lapply(unique(cls), function(x)
                colMeans(D[which(cls == x), ]))
        s <-
            lapply(unique(cls), function(x)
                cov(D[which(cls == x), ]))
    }
    plot(D[, axis], col = cls, pch = 19)
    for (k in seq_len(length(unique(cls)))) {
        # id <- which(colnames(D) == colnames(D))
        addEllipse(as.matrix(m[[k]][axis, 1]), s[[k]][axis, axis])
    }
}
