...
#'@description This function perform clustering of a univariate and multivariate continuous futures by using a gaussian mixture model.
#' @inheritParams groupMixData
#' @param D The Data to group. A matrix, or data.frame containing continuous variable(s)
...
#' #plotResults(mod_gaussian)
#'
#'@export
groupGaussianData <- function(D, K, modelType="diagonal", randomInit = 100, endIter=FALSE, nbIter=10){
#browser()
if (!is.matrix(D)){
  D <- as.matrix(D)
}
...
for(n in 1:randomInit) {
  ...
  # faster version
  means <-
    lapply(seq_len(K), function(k)colMeans(as.matrix(D[which(z[, k] == 1), ])))

  # Covariance matrix faster version
  mat_cov <-
    lapply(seq_len(K), function(k) cov(as.matrix(D[which(z[, k] == 1),])))

  # Likelihood evaluation
  lik <- matrix(0, ncol = K, nrow = nrow(D), byrow = T)
  for (k in seq_len(K)) {
    lik[, k] <- pk[, k] * dmv_gaussian(D, m = means[[k]], s = mat_cov[[k]])
  }
...
repeat{
  ...
  if (modelType == "general"){
    ...
    new_matcov <- lapply(seq_len(K), function(k) matCov_lkbk(Wk[[k]], sum(tik[, k]),nx=ncol(D)))
  }else if (modelType == "spherical"){
    new_matcov <- lapply(seq_len(K), function(k) matCov_lkI(Wk[[k]], sum(tik[, k],nx=ncol(D))))
  }else{
    stop("Choose a correct model type. 3 possible models : general, diagonal or spherical")
    break
  }
